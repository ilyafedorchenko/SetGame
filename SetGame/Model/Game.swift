...
//  }
  ...
  init() {
    ...
    changeStateForNumberOfCards(from: .inDeck, to: .onTheTable, numberOfCards: numberOfCardsToStart)
  ...
  func selectCard(cardId: Int) -> Bool {
    ...
    case .onTheTable?:
      if cards.filter({$1 == .selected}).count == numberOfCardsToSelect{
        changeStateForNumberOfCards(from: .selected, to: .onTheTable, numberOfCards: numberOfCardsToSelect)
        ...
        if cards.filter({$1 == .selected}).count == numberOfCardsToSelect && checkSet(){
          changeStateForNumberOfCards(from: .selected, to: .matched, numberOfCards: numberOfCardsToSelect)
        }
      }
